...
# remove old file
os.remove("./output/nifty1.csv") if os.path.exists("./output/nifty1.csv") else None
...
with open(
    ...
    while True:
        try:
            ...
            #limit outlier loss
            if line_data[-3] == "T9_15" and open_trade:
                if trade_type == 0 and (in_price - candle[3]) > 100:
                    print(f"==========>{line_data[0]} {trade_type} | {round(in_price - candle[3],2)}")
                    # exit
                    ex_price = candle[3]
                    open_trade = False
                    write_to_csv(
                ...
                elif trade_type == 1 and (candle[3] - in_price) > 100:
                    print(f"==========>{line_data[0]} {trade_type} | {round(candle[3] - in_price,2)}")
                    # exit
                    ex_price = candle[3]
                    open_trade = False
                    write_to_csv(
